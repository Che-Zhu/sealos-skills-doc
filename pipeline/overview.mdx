...
| 未编号环节 | 运行入口、部署模式识别、中断恢复、状态记录、清理 |

Phase 0 和 [运行入口](/pipeline/entry) 是每次运行的共同入口。[部署模式识别](/pipeline/mode-detection) 在运行入口之后执行。Phase 1 仅在 DEPLOY 模式下执行。
...
**官方模板路径：** 部署前环境检查 → 运行入口 → 部署模式识别 → 项目评估 → 唯一精确匹配 → 原样复用官方模板 → 收集模板输入 → **Phase 5.9 部署预检（YAML dry-run）** → 部署 → 运行验收 → 状态记录

**标准构建路径：** 部署前环境检查 → 运行入口 → 部署模式识别 → 项目评估 → 无可用官方模板 → 镜像与拓扑发现 → 逐服务构建或复用镜像 → 模板生成 → 应用配置 → **Phase 5.9 部署预检（YAML dry-run）** → 部署 → 运行验收 → 状态记录
...
| Phase | 名称 | 主要任务 |
| --- | --- | --- |
| [Phase 0](/pipeline/phases/phase-0-preflight) | 部署前环境检查 | 判定 skill 能否运行 |
| [Phase 1](/pipeline/phases/phase-1-assess) | 项目评估 | DEPLOY 模式下记录事实，写入 analysis.json |
