...
Sealos Deploy 一共定义 **12 个具名 Phase**（原 Phase 3 已合并入 Phase 2）：

| 类别 | 阶段 |
| --- | --- |
| 主部署生命周期 | Phase 0、1、2、4、5、5.5、5.9、6、6.5 |
...
**官方模板路径：** 部署前环境检查 → 部署模式识别 → 项目评估（快速路径）→ **Phase 5.9 部署预检** → Phase 6 收集模板输入 → 部署 → 运行验收 → 状态记录

**标准构建路径：** 部署前环境检查 → 部署模式识别 → 项目评估 → 未走快速路径 → **Phase 2**（`deployment-plan.json`）→ **Phase 5**（按需 Phase 4 构建）→ 模板生成 → 应用配置 → **Phase 5.9 部署预检（YAML dry-run）** → 部署 → 运行验收 → 状态记录
...
原 [Phase 3](/pipeline/phases/phase-3-build-prep) 已合并入 Phase 2。
