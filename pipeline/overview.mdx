---
title: "流水线总览"
description: "Sealos Deploy 从部署前检查到运行验收与更新的完整生命周期。"
...
<Note>
本章节是 `sealos-deploy` 生命周期的**规范源（SSOT）**，取代 [Sealos Deploy Lifecycle Wiki](https://github.com/labring/sealos-skills/wiki/Sealos-Deploy-Lifecycle)。
</Note>
...
| 类别 | 阶段 |
| --- | --- |
| 主部署生命周期 | Phase 0、1、1.5、2、3、4、5、5.5、6、6.5 |
| 已有应用更新 | Phase U1、U2、U3 |
| 未编号环节 | 模式识别、中断恢复、状态记录、清理 |

Phase 0 和模式识别是每次运行的共同入口；Phase 1 只在首次部署流程中执行。
...
**官方模板路径：** 部署前检查 → 模式识别 → 项目评估 → 唯一精确匹配 → 原样复用官方模板 → 收集模板输入 → 部署 → 运行验收 → 状态记录

**标准构建路径：** 部署前检查 → 模式识别 → 项目评估 → 无可用官方模板 → 镜像与拓扑发现 → 逐服务构建或复用镜像 → 模板生成 → 应用配置 → 部署 → 运行验收 → 状态记录

## 更新路径

识别已有应用后：部署前检查 → 选择重建或重启 → 应用更新 → 验证发布 → 记录成功或回滚
...
| Phase | 名称 | 主要任务 |
| --- | --- | --- |
| [Phase 0](/pipeline/phases/phase-0-preflight) | 部署前检查 | 环境、区域、登录、工作空间、源码物化 |
| [Phase 1](/pipeline/phases/phase-1-assess) | 项目评估 | 部署形态、云原生就绪度、项目事实 |
| [Phase 1.5](/pipeline/phases/phase-1-5-template) | 官方模板复用 | 精确匹配则跳到 Phase 6，否则进入 Phase 2 |
