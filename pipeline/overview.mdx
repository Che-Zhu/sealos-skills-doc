...
Sealos Deploy 一共定义 **13 个具名 Phase**：

| 类别 | 阶段 |
| --- | --- |
| 主部署生命周期 | Phase 0、1、2、3、4、5、5.5、5.9、6、6.5 |
...
**标准构建路径：** 部署前环境检查 → 部署模式识别 → 项目评估 → 未走快速路径（无精确匹配，或 `local` 有未提交更改）→ 镜像与拓扑发现 → 逐服务构建或复用镜像 → 模板生成 → 应用配置 → **Phase 5.9 部署预检（YAML dry-run）** → 部署 → 运行验收 → 状态记录
...
| Phase | 名称 | 主要任务 |
| --- | --- | --- |
| [Phase 0](/pipeline/phases/phase-0-preflight) | 部署前环境检查 | 环境、源码物化、写入 `analysis.json`（来源与 `runtime_profile`） |
